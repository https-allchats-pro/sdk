...
from allchats_sdk import FileCredentialStore, VKClient
...
async def connect_qr(account_id: str, session_file: Path) -> None:
    ...
    print(f"[vk] authorized user_id={authorized.user_id} state={authorized.state.value}")
...
async def connect_token(account_id: str, session_file: Path) -> None:
    ...
    print(f"[vk] connected state={status.state.value} user_id={status.user_id}")
    print(f"[vk] session saved → {store.path}")
    await asyncio.sleep(1)
    await client.disconnect()
...
async def connect_oauth(account_id: str, session_file: Path) -> None:
    ...
    status = await client.connect()
    print(f"[vk] oauth connected state={status.state.value} user_id={status.user_id}")
    print(f"[vk] session saved → {store.path}")
    await asyncio.sleep(1)
    await client.disconnect()
...
async def reconnect(account_id: str, session_file: Path) -> None:
    ...
    client = _build_client(account_id, store)
    status = await client.connect()
    print(f"[vk] reconnected state={status.state.value} user_id={status.user_id}")
    await asyncio.sleep(1)
    await client.disconnect()
