"""Telegram provider — multi-account runtime.

Public name: ``TelegramProvider``. Legacy alias: ``TelegramClientManager``.
"""
...
class TelegramProvider:
    ...
    async def disconnect(self, account_id: str) -> None:
        """Stop QR auth and the active session (conceptual API)."""
        await self.stop_qr(account_id)
        await self.stop_session(account_id)
...
TelegramClientManager = TelegramProvider
